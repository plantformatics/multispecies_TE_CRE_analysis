...
bedtools intersect -a ${FA}.annotated.ACR.bed -b ${FA}.fa.out.fcon.bed.sorted -wa -wb -sorted > ${FA}.ACR_TEmapped.basic.bed
...
for i in `seq 1 10000`; do
        echo "bedtools shuffle -i ${FA}.sum.bed.sorted -g ${FA}.fa.fai -excl ${FA}.sum.bed.sorted | bedtools annotate -i - -files ${FA}.intergenic.bed ${FA}.promoters.gff ${FA}.genic.gff ${FA}.downstream.gff | sort -k1,1 -k2,2n - | bedtools intersect -a - -b ${FA}.fa.out.fcon.bed.sorted -sorted -wa -wb -f 0.5 | uniq - > ./temp/$i.TE" >> commandlist.txt
done
